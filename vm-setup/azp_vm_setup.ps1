...
if ($Mode -eq "prepareseed") {
    ...
    ExecuteScriptWithMountedVhd $Vhd {
        ...
        Copy-Item "Generate-Certificates.ps1" "$($vhdmountdir)"
        Copy-Item "dswdevice.exe" "$($vhdmountdir)"
        Copy-Item "devcon.exe" "$($vhdmountdir)"
        Copy-Item "kd.exe" "$($vhdmountdir)"
        Copy-Item "livekd64.exe" "$($vhdmountdir)"
        Copy-Item "notmyfault64.exe" "$($vhdmountdir)"
        Copy-Item "CoreNet-CI-Startup.ps1" "$($vhdmountdir)"
        Copy-Item "tracepdb.exe" "$($vhdmountdir)"
        Copy-Item "wsario.exe" "$($vhdmountdir)"
